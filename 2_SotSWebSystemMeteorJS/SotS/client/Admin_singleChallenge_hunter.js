...
import './Admin_singleChallenge_hunter.html';
...
Template.Admin_singleChallenge_hunter.onCreated (function () {
    var self = this;

    this.editMode = new ReactiveVar (false);
    this.validated = new ReactiveVar (false);

    listOfChallenges = Challenges_HunterDB.find({});
...
Template.Admin_singleChallenge_hunter.helpers({ 
...
Template.Admin_singleChallenge_hunter.events ({ 
   ...
   'click .toToggleTheValidation2': function () {


    var chs = Challenges_HunterDB.find({});
    chs.forEach(element => 
        {
            if (element._id == this._id)
            {
                var validationStatus = element.validated;
                Meteor.call('toggleValidation2', this._id, 
                validationStatus,
                (error) => {
                    if (error) {
                        alert(error.error);
                        ...
                        if (validationStatus)
                        {
                            ...
                            ServerRemoveChallengeHunterToPlayFabTitleData(element._id, element.ownerPlayFabID);
                            
                        }
                        else
                        {
                            //alert("Challenge Validated :)");
                            ServerAddChallengeHunterToPlayFabTitleData(element._id, element.ownerPlayFabID);
                        }
...
function ServerRemoveChallengeHunterToPlayFabTitleData(thisId, ownerPlayFabID) {
            ...
            if(!response.data.Data || !response.data.Data[thisId]) {
                ...
                function (response, error) {
                    ...
                    } else 
                    {
                        ...
                        manageBadgeChallengesHunterCreatedForOwner(false, ownerPlayFabID);
                        alert("Hunter Challenge Revoked!");
...
function ServerAddChallengeHunterToPlayFabTitleData (thisId, ownerPlayFabID) { 
    PlayFabServerSDK.GetTitleData({}, function (response, error) 
    {
        if(error) {
            console.log("Got error getting titleData:");
            ...
            if(!response.data.Data || !response.data.Data[thisId]) {
                ...
                function (response, error) {
                    ...
                    } else 
                    {
                        ...
                        alert("Hunter Challenge Validated :)");        
                        
                        manageBadgeChallengesHunterCreatedForOwner(true, ownerPlayFabID);
                    }
...
function manageBadgeChallengesHunterCreatedForOwner(increment, ownerPlayFabID) 
{
