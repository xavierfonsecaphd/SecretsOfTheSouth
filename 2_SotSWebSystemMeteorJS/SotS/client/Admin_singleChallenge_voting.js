...
import './Admin_singleChallenge_voting.html';
...
Template.Admin_singleChallenge_voting.onCreated (function () {
    var self = this;

    this.editMode = new ReactiveVar (false);
    this.validated = new ReactiveVar (false);

    listOfChallenges = Challenges_VotingDB.find({});
...
Template.Admin_singleChallenge_voting.events ({ 
    ...
    'submit .info-challenge-edit_voting'(event) {
        event.preventDefault();
        const target = event.target;
    
        const name = target.name;
        const description = target.description;
        const typeOfChallengeIndex = 3;
        var lat;
        var lng;
        ...
        const listOfImagesAndVotes = this.listOfImagesAndVotes; // keep the value it had so far
        ...
        listOfChallenges.forEach(element => 
                {
                    if (element._id == this._id)
                    {
                        ...
                        Meteor.call('updateChallenge_Voting', this._id, 
                        name.value, 
                        description.value,
                        ownerPlayFabID,
                        ...
                        (error) => {
                            if (error) {
                                ...
                                alert("Voting Challenge updated successfully :) Reloading page...");
                            }
                        });
      ...
      'click .close-edit-challenge_voting': function() { 
            Template.instance().editMode.set(false);
   },
   'click .toToggleTheValidation3': function () {


    var chs = Challenges_VotingDB.find({});
    chs.forEach(element => 
        {
            if (element._id == this._id)
            {
                var validationStatus = element.validated;
                Meteor.call('toggleValidation3', this._id, 
                validationStatus,
                (error) => {
                    if (error) {
                        alert(error.error);
                        ...
                        if (validationStatus)
                        {
                            ...
                            ServerRemoveChallengeVotingToPlayFabTitleData(element._id, element.ownerPlayFabID);
                            
                        }
                        else
                        {
                            //alert("Challenge Validated :)");
                            ServerAddChallengeVotingToPlayFabTitleData(element._id, element.ownerPlayFabID);
                        }
...
function ServerRemoveChallengeVotingToPlayFabTitleData(thisId, ownerPlayFabID) {
            ...
            if(!response.data.Data || !response.data.Data[thisId]) {
                ...
                function (response, error) {
                    ...
                    } else 
                    {
                        ...
                        alert("Voting Challenge Revoked!");
...
function ServerAddChallengeVotingToPlayFabTitleData (thisId, ownerPlayFabID) { 
    PlayFabServerSDK.GetTitleData({}, function (response, error) 
    {
        if(error) {
            console.log("Got error getting titleData:");
            ...
            if(!response.data.Data || !response.data.Data[thisId]) {
                ...
                function (response, error) {
                    ...
                    } else 
                    {
                        ...
                        alert("Voting Challenge Validated :)");        
                        
                        manageBadgeChallengesVotingCreatedForOwner(true, ownerPlayFabID);
                    }
...
function manageBadgeChallengesVotingCreatedForOwner(increment, ownerPlayFabID) 
{
